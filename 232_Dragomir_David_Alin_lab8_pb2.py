...
def elem_opus(lista, joc_opus):
	mt = set(lista)
	if len(mt) == 1:
		castigator = list(mt)[0]
		if castigator != joc_opus:
			return 1
		else :
			return 0
	else :
...
class Joc:
    ...
    def final(self):
        ...
        for i in range (self.NR_LINII):
            tex = self.NR_COLOANE - self.NR_CONNECT
            for j in range(tex):
                a = []
                for k in range(self.NR_CONNECT):
                    a.append(self.matr[i * (j + k)])
                rez = elem_identice(list(a))
                if (rez == True):
                    break
        if(rez == False):
            for i in range (self.NR_COLOANE):
                tex = self.NR_LINII - self.NR_CONNECT
                for j in range(tex):
                    a = []
                    for k in range (self.NR_CONNECT):
                        a.append(self.matr[i * (j + k)])
                    ...
                    if 0 <= coloana < Joc.NR_COLOANE:
                        if stare_curenta.tabla_joc.matr[coloana] != Joc.GOL:
                            ...
                            for linie in range(Joc.NR_LINII - 1, -1, -1):
                                calc = linie * Joc.NR_COLOANE
                                if stare_curenta.tabla_joc.matr[calc + coloana] == Joc.GOL:
                                    break
                            raspuns_valid = True
                except ValueError:
